...
def greedy_strategy_selector(numExplorer, numActiveAlien, agent_energy_level, agent_hp_level):
    ...
    tmp = sorted(greedy_single1.Greedy_DecisionMaking(numExplorer, numActiveAlien, agent_energy_level, agent_hp_level).items(), key = lambda kv:(kv[1], kv[0]))[-1]
    print(tmp[0])
...
def gut_explore_game():
    ...
    alien1_label = r.axes.text(x[0,1],x[1,1]+0.25,"alien",fontsize=font_size1, color='r',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    ...
    # alien2_label = r.axes.text(x[0,2],x[1,2]+0.25,"alien",fontsize=font_size1, color='r',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    ...
    for t in range(iterations):
        ...
        if numActiveAlien == 0:
            ...
            #Alien
            alienpoint1 = alienpoints[:,state].reshape((2,1))
            ...
            for i in range(N):
                if i == 0:
                    # strategy = greedy_strategy_selector(numActiveAlien, x[:2, i], goalPos)
                    strategy = greedy_strategy_selector(N-1, numActiveAlien, agent_energy_level, agent_hp_level)
                    print(strategy)
        ...
        for i in range(N-1):
            ...
            #     # numActiveAlien += 1
            #     agent_hp_level[i] -= unit_alien_attacking_hp_cost * 2.4;
        ...
        # print(np.linalg.norm(x[:2,[0]] - waypoints[:2, -1]))
