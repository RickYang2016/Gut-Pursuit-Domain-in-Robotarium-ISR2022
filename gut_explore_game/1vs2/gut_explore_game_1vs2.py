...
def gut_explore_game():
    ...
    waypoints = np.array([[-0.36, 0.496, 1.158],[-0.36, -0.653, 0.8]]) #Waypoints the leader moves to.
    ...
    alienpoints1 = np.array([[-0.591, 0.12, 0.727],[-0.1, -0.76, -0.174]]) # attacking
    alienpoints2 = np.array([[-0.13, 0.14, 0.842],[-0.161, -0.515, 0.174]]) # attacking
    ...
    for t in range(iterations):
        ...
        if numActiveAlien == 0:
            ...
            explorer_strategy, _ = gut_single2.GUT_DecisionMaking(N-2, numActiveAlien, agent_energy_level, agent_hp_level)
        ...
        #     alienpoint2 = alienpoints2[:,state].reshape((2,1))
        # else:
        #     alienpoint1 = alienpoints1[:,state-1].reshape((2,1))
        #     alienpoint2 = alienpoints2[:,state-1].reshape((2,1))
