...
def second_level_mne(e, a, strategyProbablity):
    for s in range(len(strategyProbablity)):
        if s == 0:
            e['forward'] = strategyProbablity[s]
        elif s ==1:
            e['avoid'] = strategyProbablity[s]
        elif s == 2:
...
def HPUtility(numAttackingAgent, numAttackingAdversary, agent_hp_level, explorerStrategy, alienStrategy):
    ...
    if explorerStrategy == 'forward' and alienStrategy == 'follow':
        explorerArea = agentArea * 1.5
        alienArea = agentArea * 1.2
    elif explorerStrategy == 'forward' and alienStrategy == 'back':
        explorerArea = agentArea * 1.5
        alienArea = agentArea * 0.8
    elif explorerStrategy == 'avoid' and alienStrategy == 'follow':
        explorerArea = agentArea * 0.8
        alienArea = agentArea * 1.2
    elif explorerStrategy == 'avoid' and alienStrategy == 'back':
        explorerArea = agentArea * 0.8
        alienArea = agentArea * 0.8
...
def GUT_DecisionMaking(numExplorer, numActiveAlien, agent_energy_level, agent_hp_level):
    ...
    x11 = round(HPUtility(numExplorer, numActiveAlien, agent_hp_level, 'forward', 'follow'))
    x12 = round(HPUtility(numExplorer, numActiveAlien, agent_hp_level, 'forward', 'back'))
    x21 = round(HPUtility(numExplorer, numActiveAlien, agent_hp_level, 'avoid', 'follow'))
    x22 = round(HPUtility(numExplorer, numActiveAlien, agent_hp_level, 'avoid', 'back'))

    y11 = round(HPUtility(numActiveAlien, numExplorer, agent_hp_level, 'forward', 'follow'))
    y12 = round(HPUtility(numActiveAlien, numExplorer, agent_hp_level, 'forward', 'back'))
    y21 = round(HPUtility(numActiveAlien, numExplorer, agent_hp_level, 'avoid', 'follow'))
    y22 = round(HPUtility(numActiveAlien, numExplorer, agent_hp_level, 'avoid', 'back'))
    ...
    explorer_gut_cpt['e11a11e21t'] = GUT_CPT(e1['attacking'], a1['attacking'], e21, 'forward')
    explorer_gut_cpt['e11a11e21l'] = GUT_CPT(e1['attacking'], a1['attacking'], e21, 'avoid')
    explorer_gut_cpt['e11a12e22t'] = GUT_CPT(e1['attacking'], a1['defending'], e22, 'forward')
    explorer_gut_cpt['e11a12e22l'] = GUT_CPT(e1['attacking'], a1['defending'], e22, 'avoid')

    explorer_gut_cpt['e12a11e23t'] = GUT_CPT(e1['defending'], a1['attacking'], e23, 'forward')
    explorer_gut_cpt['e12a11e23l'] = GUT_CPT(e1['defending'], a1['attacking'], e23, 'avoid')
    explorer_gut_cpt['e12a12e24t'] = GUT_CPT(e1['defending'], a1['defending'], e24, 'forward')
    explorer_gut_cpt['e12a12e24l'] = GUT_CPT(e1['defending'], a1['defending'], e24, 'avoid')
