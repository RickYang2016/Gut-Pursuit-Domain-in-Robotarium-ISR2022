...
def gut_explore_game():
    ...
    N=2 #Number of robots to use, this must stay 4 unless the Laplacian is changed.

    waypoints = np.array([[-0.06, 1.158],[-0.06, 0.8]]) #Waypoints the leader moves to.
    alienpoints = np.array([[0.035, 0.807],[0.121, 0.8]]) # attacking
    # alienpoints = np.array([[0.035, 0.321],[0.121, 0.35]]) # defending
    ...
    initial_conditions = np.array([[-1.4, -1],[-0.8, 0.121],[0, 0]])
    ...
    alien_controller = create_si_position_controller(velocity_magnitude_limit=0.09)
    ...
    # goal_caption = ['Encounter Area{0}'.format(ii) for ii in range(waypoints.shape[1])]
    # goal_caption = ['Encounter', 'Treasure']
    goal_caption = ['Treasure']

    #Plot text for caption
    waypoint_text = r.axes.text(waypoints[0, 1], waypoints[1, 1], goal_caption[0], fontsize=font_size, color='k',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=-2)
    g = r.axes.scatter(waypoints[0, 1], waypoints[1, 1], s=marker_size_goal, marker='s', facecolors='none',edgecolors=CM1[1,:],linewidth=line_width,zorder=-2)
    robot_markers = [r.axes.scatter(waypoints[0,ii], waypoints[1,ii], s=marker_size_robot, marker='o', facecolors='none',edgecolors=CM2[ii,:],linewidth=line_width) 
    ...
    alien_label = r.axes.text(x[0,1],x[1,1]+0.25,"alien",fontsize=font_size1, color='r',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    alien_energy_label = r.axes.text(x[0,1],x[1,1]+0.2,"NRG: ",fontsize=font_size2, color='c',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    alien_hp_label = r.axes.text(x[0,1],x[1,1]+0.15,"HP: ",fontsize=font_size2, color='m',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    ...
    for t in range(iterations):
        ...
        alien_label.set_position([xi[0,1],xi[1,1]+0.25])
        alien_label.set_fontsize(determine_font_size(r,font_size_m1))
        alien_energy_label.set_position([xi[0,1],xi[1,1]+0.2])
        alien_energy_label.set_fontsize(determine_font_size(r,font_size_m2))
        alien_energy_label.set_text("NRG: " + str(round(agent_energy_level[1], 2)))
        alien_hp_label.set_position([xi[0,1],xi[1,1]+0.15])
        alien_hp_label.set_fontsize(determine_font_size(r,font_size_m2))
        alien_hp_label.set_text("HP: " + str(round(agent_hp_level[1], 2)))
        ...
        if numActiveAlien == 0:
            L = graph_laplacian(N-1, L, 'patroling')
        else:
            explorer_strategy, _ = gut_single1.GUT_DecisionMaking(N-1, numActiveAlien, agent_energy_level, agent_hp_level)
            L = graph_laplacian(N-1, L, explorer_strategy[0])
        ...
        #freer
        alienpoint = alienpoints[:,state].reshape((2,1))

        # dxi[:,[0]] = leader_controller(x[:2,[0]], waypoint)
        dxi[:,[N-1]] = alien_controller(x[:2,[-1]], alienpoint)
        dxi[:,[0]] = leader_controller(x[:2,[0]], waypoint)
